...
// ---------------------------------------------------------------------------
// Raw data types (from Inference.net API)
// ---------------------------------------------------------------------------

interface InferencenetModel {
  id: string;
  object: string;
  created: number;
  owned_by: string;
...
async function fetchModels(): Promise<InferencenetModel[]> {
  const response = await fetch("https://inference.net/api/v1/models");
  if (!response.ok) {
    throw new Error(`Failed to fetch Inference.net models: ${response.status}`);
  }
  const data = (await response.json()) as { data: InferencenetModel[] };
  return data.data;
}
...
const pipeline: ScrapePipeline = {
  discover: {
    source: {
      url: "https://inference.net/api/v1/models",
      type: "api",
      description: "Inference.net /v1/models API — dynamic model discovery",
    },
  ...
  extractPricing: {
    source: {
      url: "https://inference.net/api/v1/models",
      type: "api",
      description: "Inference.net API — pricing not available via API, omitted",
    },
  ...
  extractLimits: {
    source: {
      url: "https://inference.net/api/v1/models",
      type: "api",
      description: "Inference.net API — limits not available via API, omitted",
    },
  ...
  extractModalities: {
    source: {
      url: "https://inference.net/api/v1/models",
      type: "api",
      description: "Inference.net API — modalities not available via API, omitted",
    },
  ...
  extractFeatures: {
    source: {
      url: "https://inference.net/api/v1/models",
      type: "api",
      description: "Inference.net API — features not available via API, omitted",
    },
  ...
  extractDates: {
    source: {
      url: "https://inference.net/api/v1/models",
      type: "api",
      description: "Inference.net API — created timestamp for dates",
    },
    execute: async (models: DiscoveredModel[]): Promise<Map<string, ExtractedDates>> => {
      const datesMap = new Map<string, ExtractedDates>();

      for (const m of models) {
        const raw = m.raw as InferencenetModel;
