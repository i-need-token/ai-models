...
// ---------------------------------------------------------------------------
// Raw data types (from Baseten API)
// ---------------------------------------------------------------------------
...
async function fetchModels(): Promise<BasetenModel[]> {
  const response = await fetch("https://model-api.baseten.co/v1/models");
  if (!response.ok) {
    throw new Error(`Failed to fetch Baseten models: ${response.status}`);
  }
  const data = (await response.json()) as { data: BasetenModel[] };
  return data.data;
}
...
const pipeline: ScrapePipeline = {
  discover: {
    source: {
      url: "https://model-api.baseten.co/v1/models",
      type: "api",
      description: "Baseten /v1/models API — dynamic model discovery",
    },
  ...
  extractPricing: {
    source: {
      url: "https://model-api.baseten.co/v1/models",
      type: "api",
      description: "Baseten API — pricing not available via API, omitted",
    },
  ...
  extractLimits: {
    source: {
      url: "https://model-api.baseten.co/v1/models",
      type: "api",
      description: "Baseten API — limits not available via API, omitted",
    },
  ...
  extractModalities: {
    source: {
      url: "https://model-api.baseten.co/v1/models",
      type: "api",
      description: "Baseten API — modalities not available via API, omitted",
    },
  ...
  extractFeatures: {
    source: {
      url: "https://model-api.baseten.co/v1/models",
      type: "api",
      description: "Baseten API — features not available via API, omitted",
    },
  ...
  extractDates: {
    source: {
      url: "https://model-api.baseten.co/v1/models",
      type: "api",
      description: "Baseten API — created timestamp for dates",
    },
    execute: async (models: DiscoveredModel[]): Promise<Map<string, ExtractedDates>> => {
      const datesMap = new Map<string, ExtractedDates>();

      for (const m of models) {
        const raw = m.raw as BasetenModel;
