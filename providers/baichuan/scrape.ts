...
// ---------------------------------------------------------------------------
// Raw data types (from Baichuan AI API)
// ---------------------------------------------------------------------------

interface BaichuanModel {
  id: string;
  object: string;
  created: number;
  owned_by: string;
...
async function fetchModels(): Promise<BaichuanModel[]> {
  const response = await fetch("https://api.baichuan-ai.com/v1/models");
  if (!response.ok) {
    throw new Error(`Failed to fetch Baichuan AI models: ${response.status}`);
  }
  const data = (await response.json()) as { data: BaichuanModel[] };
  return data.data;
}
...
const pipeline: ScrapePipeline = {
  discover: {
    source: {
      ...
      type: "api",
      description: "Baichuan AI /v1/models API — dynamic model discovery",
    },
  ...
  extractPricing: {
    source: {
      url: "https://api.baichuan-ai.com/v1/models",
      type: "api",
      description: "Baichuan AI API — pricing not available via API, omitted",
    },
  ...
  extractLimits: {
    source: {
      url: "https://api.baichuan-ai.com/v1/models",
      type: "api",
      description: "Baichuan AI API — limits not available via API, omitted",
    },
  ...
  extractModalities: {
    source: {
      url: "https://api.baichuan-ai.com/v1/models",
      type: "api",
      description: "Baichuan AI API — modalities not available via API, omitted",
    },
  ...
  extractFeatures: {
    source: {
      url: "https://api.baichuan-ai.com/v1/models",
      type: "api",
      description: "Baichuan AI API — features not available via API, omitted",
    },
  ...
  extractDates: {
    source: {
      url: "https://api.baichuan-ai.com/v1/models",
      type: "api",
      description: "Baichuan AI API — created timestamp for dates",
    },
    execute: async (models: DiscoveredModel[]): Promise<Map<string, ExtractedDates>> => {
      const datesMap = new Map<string, ExtractedDates>();

      for (const m of models) {
        const raw = m.raw as BaichuanModel;
