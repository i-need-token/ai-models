...
// ---------------------------------------------------------------------------
// Raw data types (from GMI Cloud API)
// ---------------------------------------------------------------------------

interface GmicloudModel {
  id: string;
  object: string;
  created: number;
  owned_by: string;
...
async function fetchModels(): Promise<GmicloudModel[]> {
  const response = await fetch("https://api.gmi-serving.com/v1/models");
  if (!response.ok) {
    throw new Error(`Failed to fetch GMI Cloud models: ${response.status}`);
  }
  const data = (await response.json()) as { data: GmicloudModel[] };
  return data.data;
}
...
const pipeline: ScrapePipeline = {
  discover: {
    source: {
      ...
      type: "api",
      description: "GMI Cloud /v1/models API — dynamic model discovery",
    },
  ...
  extractPricing: {
    source: {
      url: "https://api.gmi-serving.com/v1/models",
      type: "api",
      description: "GMI Cloud API — pricing not available via API, omitted",
    },
  ...
  extractLimits: {
    source: {
      url: "https://api.gmi-serving.com/v1/models",
      type: "api",
      description: "GMI Cloud API — limits not available via API, omitted",
    },
  ...
  extractModalities: {
    source: {
      url: "https://api.gmi-serving.com/v1/models",
      type: "api",
      description: "GMI Cloud API — modalities not available via API, omitted",
    },
  ...
  extractFeatures: {
    source: {
      url: "https://api.gmi-serving.com/v1/models",
      type: "api",
      description: "GMI Cloud API — features not available via API, omitted",
    },
  ...
  extractDates: {
    source: {
      url: "https://api.gmi-serving.com/v1/models",
      type: "api",
      description: "GMI Cloud API — created timestamp for dates",
    },
    execute: async (models: DiscoveredModel[]): Promise<Map<string, ExtractedDates>> => {
      const datesMap = new Map<string, ExtractedDates>();

      for (const m of models) {
        const raw = m.raw as GmicloudModel;
