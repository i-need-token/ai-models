...
// ---------------------------------------------------------------------------
// Raw data types (from Voyage AI API)
// ---------------------------------------------------------------------------
...
async function fetchModels(): Promise<VoyageModel[]> {
  const response = await fetch("https://api.voyageai.com/v1/models");
  if (!response.ok) {
    throw new Error(`Failed to fetch Voyage AI models: ${response.status}`);
  }
  const data = (await response.json()) as { data: VoyageModel[] };
  return data.data;
}
...
const pipeline: ScrapePipeline = {
  discover: {
    source: {
      ...
      type: "api",
      description: "Voyage AI /v1/models API — dynamic model discovery",
    },
  ...
  extractPricing: {
    source: {
      url: "https://api.voyageai.com/v1/models",
      type: "api",
      description: "Voyage AI API — pricing not available via API, omitted",
    },
  ...
  extractLimits: {
    source: {
      url: "https://api.voyageai.com/v1/models",
      type: "api",
      description: "Voyage AI API — limits not available via API, omitted",
    },
  ...
  extractModalities: {
    source: {
      url: "https://api.voyageai.com/v1/models",
      type: "api",
      description: "Voyage AI API — modalities not available via API, omitted",
    },
  ...
  extractFeatures: {
    source: {
      url: "https://api.voyageai.com/v1/models",
      type: "api",
      description: "Voyage AI API — features not available via API, omitted",
    },
  ...
  extractDates: {
    source: {
      url: "https://api.voyageai.com/v1/models",
      type: "api",
      description: "Voyage AI API — created timestamp for dates",
    },
    execute: async (models: DiscoveredModel[]): Promise<Map<string, ExtractedDates>> => {
      const datesMap = new Map<string, ExtractedDates>();

      for (const m of models) {
        const raw = m.raw as VoyageModel;
