...
// ---------------------------------------------------------------------------
// Raw data types (from Sarvam AI API)
// ---------------------------------------------------------------------------
...
async function fetchModels(): Promise<SarvamModel[]> {
  const response = await fetch("https://api.sarvam.ai/v1/models");
  if (!response.ok) {
    throw new Error(`Failed to fetch Sarvam AI models: ${response.status}`);
  }
  const data = (await response.json()) as { data: SarvamModel[] };
  return data.data;
}
...
const pipeline: ScrapePipeline = {
  discover: {
    source: {
      ...
      type: "api",
      description: "Sarvam AI /v1/models API — dynamic model discovery",
    },
  ...
  extractPricing: {
    source: {
      url: "https://api.sarvam.ai/v1/models",
      type: "api",
      description: "Sarvam AI API — pricing not available via API, omitted",
    },
  ...
  extractLimits: {
    source: {
      url: "https://api.sarvam.ai/v1/models",
      type: "api",
      description: "Sarvam AI API — limits not available via API, omitted",
    },
  ...
  extractModalities: {
    source: {
      url: "https://api.sarvam.ai/v1/models",
      type: "api",
      description: "Sarvam AI API — modalities not available via API, omitted",
    },
  ...
  extractFeatures: {
    source: {
      url: "https://api.sarvam.ai/v1/models",
      type: "api",
      description: "Sarvam AI API — features not available via API, omitted",
    },
  ...
  extractDates: {
    source: {
      url: "https://api.sarvam.ai/v1/models",
      type: "api",
      description: "Sarvam AI API — created timestamp for dates",
    },
    execute: async (models: DiscoveredModel[]): Promise<Map<string, ExtractedDates>> => {
      const datesMap = new Map<string, ExtractedDates>();

      for (const m of models) {
        const raw = m.raw as SarvamModel;
  ...
  deriveFamily: {
    execute: (modelId: string): string => {
      ...
      const rules: Array<{ pattern: RegExp; family: string }> = [
        { pattern: /sarvam-m/i, family: "sarvam-m" },
