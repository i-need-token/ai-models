...
const provider = defineProvider({
  ...
  api_docs: "https://docs.dinference.com",
  apis: {
...
// ---------------------------------------------------------------------------
// Raw data types (from D-Inference API)
// ---------------------------------------------------------------------------

interface DinferenceModel {
  id: string;
  object: string;
  created: number;
  owned_by: string;
...
async function fetchModels(): Promise<DinferenceModel[]> {
  const response = await fetch("https://api.dinference.com/v1/models");
  if (!response.ok) {
    throw new Error(`Failed to fetch D-Inference models: ${response.status}`);
  }
  const data = (await response.json()) as { data: DinferenceModel[] };
  return data.data;
}
...
const pipeline: ScrapePipeline = {
  discover: {
    source: {
      url: "https://api.dinference.com/v1/models",
      type: "api",
      description: "D-Inference /v1/models API — dynamic model discovery",
    },
  ...
  extractPricing: {
    source: {
      url: "https://api.dinference.com/v1/models",
      type: "api",
      description: "D-Inference API — pricing not available via API, omitted",
    },
  ...
  extractLimits: {
    source: {
      url: "https://api.dinference.com/v1/models",
      type: "api",
      description: "D-Inference API — limits not available via API, omitted",
    },
  ...
  extractModalities: {
    source: {
      url: "https://api.dinference.com/v1/models",
      type: "api",
      description: "D-Inference API — modalities not available via API, omitted",
    },
  ...
  extractFeatures: {
    source: {
      url: "https://api.dinference.com/v1/models",
      type: "api",
      description: "D-Inference API — features not available via API, omitted",
    },
  ...
  extractDates: {
    source: {
      url: "https://api.dinference.com/v1/models",
      type: "api",
      description: "D-Inference API — created timestamp for dates",
    },
    execute: async (models: DiscoveredModel[]): Promise<Map<string, ExtractedDates>> => {
      const datesMap = new Map<string, ExtractedDates>();

      for (const m of models) {
        const raw = m.raw as DinferenceModel;
...
export async function scrape(): Promise<ScrapeResult> {
  ...
  console.log(`  D-Inference: ${models.length} models`);
