...
const provider = defineProvider({
  ...
  apis: {
    openai: "https://api.xiaomi.ai/v1",
  },
});

// ---------------------------------------------------------------------------
// Raw data types (from Xiaomi API)
// ---------------------------------------------------------------------------
...
async function fetchModels(): Promise<XiaomiModel[]> {
  const response = await fetch("https://api.xiaomi.ai/v1/models");
  if (!response.ok) {
    throw new Error(`Failed to fetch Xiaomi models: ${response.status}`);
  }
  const data = (await response.json()) as { data: XiaomiModel[] };
  return data.data;
}
...
const pipeline: ScrapePipeline = {
  discover: {
    source: {
      url: "https://api.xiaomi.ai/v1/models",
      type: "api",
      description: "Xiaomi /v1/models API — dynamic model discovery",
    },
  ...
  extractPricing: {
    source: {
      url: "https://api.xiaomi.ai/v1/models",
      type: "api",
      description: "Xiaomi API — pricing not available via API, omitted",
    },
  ...
  extractLimits: {
    source: {
      url: "https://api.xiaomi.ai/v1/models",
      type: "api",
      description: "Xiaomi API — limits not available via API, omitted",
    },
  ...
  extractModalities: {
    source: {
      url: "https://api.xiaomi.ai/v1/models",
      type: "api",
      description: "Xiaomi API — modalities not available via API, omitted",
    },
  ...
  extractFeatures: {
    source: {
      url: "https://api.xiaomi.ai/v1/models",
      type: "api",
      description: "Xiaomi API — features not available via API, omitted",
    },
  ...
  extractDates: {
    source: {
      url: "https://api.xiaomi.ai/v1/models",
      type: "api",
      description: "Xiaomi API — created timestamp for dates",
    },
    execute: async (models: DiscoveredModel[]): Promise<Map<string, ExtractedDates>> => {
      const datesMap = new Map<string, ExtractedDates>();

      for (const m of models) {
        const raw = m.raw as XiaomiModel;
