...
// ---------------------------------------------------------------------------
// Raw data types (from StepFun API)
// ---------------------------------------------------------------------------
...
async function fetchModels(): Promise<StepfunModel[]> {
  const response = await fetch("https://api.stepfun.com/v1/models");
  if (!response.ok) {
    throw new Error(`Failed to fetch StepFun models: ${response.status}`);
  }
  const data = (await response.json()) as { data: StepfunModel[] };
  return data.data;
}
...
const pipeline: ScrapePipeline = {
  discover: {
    source: {
      ...
      type: "api",
      description: "StepFun /v1/models API — dynamic model discovery",
    },
  ...
  extractPricing: {
    source: {
      url: "https://api.stepfun.com/v1/models",
      type: "api",
      description: "StepFun API — pricing not available via API, omitted",
    },
  ...
  extractLimits: {
    source: {
      url: "https://api.stepfun.com/v1/models",
      type: "api",
      description: "StepFun API — limits not available via API, omitted",
    },
  ...
  extractModalities: {
    source: {
      url: "https://api.stepfun.com/v1/models",
      type: "api",
      description: "StepFun API — modalities not available via API, omitted",
    },
  ...
  extractFeatures: {
    source: {
      url: "https://api.stepfun.com/v1/models",
      type: "api",
      description: "StepFun API — features not available via API, omitted",
    },
  ...
  extractDates: {
    source: {
      url: "https://api.stepfun.com/v1/models",
      type: "api",
      description: "StepFun API — created timestamp for dates",
    },
    execute: async (models: DiscoveredModel[]): Promise<Map<string, ExtractedDates>> => {
      const datesMap = new Map<string, ExtractedDates>();

      for (const m of models) {
        const raw = m.raw as StepfunModel;
