...
// ---------------------------------------------------------------------------
// Raw data types (from Nebius Token Factory API)
// ---------------------------------------------------------------------------
...
async function fetchModels(): Promise<NebiusModel[]> {
  const response = await fetch("https://api.tokenfactory.nebius.com/v1/models");
  if (!response.ok) {
    throw new Error(`Failed to fetch Nebius Token Factory models: ${response.status}`);
  }
  const data = (await response.json()) as { data: NebiusModel[] };
  return data.data;
}
...
const pipeline: ScrapePipeline = {
  discover: {
    source: {
      ...
      type: "api",
      description: "Nebius Token Factory /v1/models API — dynamic model discovery",
    },
  ...
  extractPricing: {
    source: {
      url: "https://api.tokenfactory.nebius.com/v1/models",
      type: "api",
      description: "Nebius Token Factory API — pricing not available via API, omitted",
    },
  ...
  extractLimits: {
    source: {
      url: "https://api.tokenfactory.nebius.com/v1/models",
      type: "api",
      description: "Nebius Token Factory API — limits not available via API, omitted",
    },
  ...
  extractModalities: {
    source: {
      url: "https://api.tokenfactory.nebius.com/v1/models",
      type: "api",
      description: "Nebius Token Factory API — modalities not available via API, omitted",
    },
  ...
  extractFeatures: {
    source: {
      url: "https://api.tokenfactory.nebius.com/v1/models",
      type: "api",
      description: "Nebius Token Factory API — features not available via API, omitted",
    },
  ...
  extractDates: {
    source: {
      url: "https://api.tokenfactory.nebius.com/v1/models",
      type: "api",
      description: "Nebius Token Factory API — created timestamp for dates",
    },
    execute: async (models: DiscoveredModel[]): Promise<Map<string, ExtractedDates>> => {
      const datesMap = new Map<string, ExtractedDates>();

      for (const m of models) {
        const raw = m.raw as NebiusModel;
