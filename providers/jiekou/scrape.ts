...
async function fetchModels(): Promise<JiekouModel[]> {
  const response = await fetch("https://api.jiekou.ai/v1/models");
  if (!response.ok) {
    throw new Error(`Failed to fetch 接口 AI models: ${response.status}`);
  }
  const data = (await response.json()) as { data: JiekouModel[] };
  return data.data;
}
...
const pipeline: ScrapePipeline = {
  discover: {
    source: {
      url: "https://api.jiekou.ai/v1/models",
      type: "api",
      description: "接口 AI /v1/models API — dynamic model discovery",
    },
  ...
  extractPricing: {
    source: {
      url: "https://api.jiekou.ai/v1/models",
      type: "api",
      description: "接口 AI API — pricing not available via API, omitted",
    },
  ...
  extractLimits: {
    source: {
      url: "https://api.jiekou.ai/v1/models",
      type: "api",
      description: "接口 AI API — limits not available via API, omitted",
    },
  ...
  extractModalities: {
    source: {
      url: "https://api.jiekou.ai/v1/models",
      type: "api",
      description: "接口 AI API — modalities not available via API, omitted",
    },
  ...
  extractFeatures: {
    source: {
      url: "https://api.jiekou.ai/v1/models",
      type: "api",
      description: "接口 AI API — features not available via API, omitted",
    },
  ...
  extractDates: {
    source: {
      url: "https://api.jiekou.ai/v1/models",
      type: "api",
      description: "接口 AI API — created timestamp for dates",
    },
    execute: async (models: DiscoveredModel[]): Promise<Map<string, ExtractedDates>> => {
      const datesMap = new Map<string, ExtractedDates>();

      for (const m of models) {
        const raw = m.raw as JiekouModel;
...
export async function scrape(): Promise<ScrapeResult> {
  ...
  console.log(`  接口 AI: ${models.length} models`);
