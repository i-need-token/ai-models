...
// ---------------------------------------------------------------------------
// Raw data types (from Scaleway API)
// ---------------------------------------------------------------------------
...
async function fetchModels(): Promise<ScalewayModel[]> {
  ...
  if (!response.ok) {
    throw new Error(`Failed to fetch Scaleway models: ${response.status}`);
  }
  const data = (await response.json()) as { data: ScalewayModel[] };
  return data.data;
}
...
const pipeline: ScrapePipeline = {
  discover: {
    source: {
      ...
      type: "api",
      description: "Scaleway /v1/models API — dynamic model discovery",
    },
  ...
  extractPricing: {
    source: {
      url: "https://api.scaleway.ai/v1/models",
      type: "api",
      description: "Scaleway API — pricing not available via API, omitted",
    },
  ...
  extractLimits: {
    source: {
      url: "https://api.scaleway.ai/v1/models",
      type: "api",
      description: "Scaleway API — limits not available via API, omitted",
    },
  ...
  extractModalities: {
    source: {
      url: "https://api.scaleway.ai/v1/models",
      type: "api",
      description: "Scaleway API — modalities not available via API, omitted",
    },
  ...
  extractFeatures: {
    source: {
      url: "https://api.scaleway.ai/v1/models",
      type: "api",
      description: "Scaleway API — features not available via API, omitted",
    },
  ...
  extractDates: {
    source: {
      url: "https://api.scaleway.ai/v1/models",
      type: "api",
      description: "Scaleway API — created timestamp for dates",
    },
    execute: async (models: DiscoveredModel[]): Promise<Map<string, ExtractedDates>> => {
      const datesMap = new Map<string, ExtractedDates>();

      for (const m of models) {
        const raw = m.raw as ScalewayModel;
