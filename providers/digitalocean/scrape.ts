...
// ---------------------------------------------------------------------------
// Raw data types (from DigitalOcean API)
// ---------------------------------------------------------------------------
...
async function fetchModels(): Promise<DigitaloceanModel[]> {
  const response = await fetch("https://inference.do.ai/v1/models");
  if (!response.ok) {
    throw new Error(`Failed to fetch DigitalOcean models: ${response.status}`);
  }
  const data = (await response.json()) as { data: DigitaloceanModel[] };
  return data.data;
}
...
const pipeline: ScrapePipeline = {
  discover: {
    source: {
      url: "https://inference.do.ai/v1/models",
      type: "api",
      description: "DigitalOcean /v1/models API — dynamic model discovery",
    },
  ...
  extractPricing: {
    source: {
      url: "https://inference.do.ai/v1/models",
      type: "api",
      description: "DigitalOcean API — pricing not available via API, omitted",
    },
  ...
  extractLimits: {
    source: {
      url: "https://inference.do.ai/v1/models",
      type: "api",
      description: "DigitalOcean API — limits not available via API, omitted",
    },
  ...
  extractModalities: {
    source: {
      url: "https://inference.do.ai/v1/models",
      type: "api",
      description: "DigitalOcean API — modalities not available via API, omitted",
    },
  ...
  extractFeatures: {
    source: {
      url: "https://inference.do.ai/v1/models",
      type: "api",
      description: "DigitalOcean API — features not available via API, omitted",
    },
  ...
  extractDates: {
    source: {
      url: "https://inference.do.ai/v1/models",
      type: "api",
      description: "DigitalOcean API — created timestamp for dates",
    },
    execute: async (models: DiscoveredModel[]): Promise<Map<string, ExtractedDates>> => {
      const datesMap = new Map<string, ExtractedDates>();

      for (const m of models) {
        const raw = m.raw as DigitaloceanModel;
