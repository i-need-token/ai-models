...
// ---------------------------------------------------------------------------
// Raw data types (from Qiniu AI API)
// ---------------------------------------------------------------------------
...
async function fetchModels(): Promise<QiniuModel[]> {
  const response = await fetch("https://api.qnaigc.com/v1/models");
  if (!response.ok) {
    throw new Error(`Failed to fetch Qiniu AI models: ${response.status}`);
  }
  const data = (await response.json()) as { data: QiniuModel[] };
  return data.data;
}
...
const pipeline: ScrapePipeline = {
  discover: {
    source: {
      ...
      type: "api",
      description: "Qiniu AI /v1/models API — dynamic model discovery",
    },
  ...
  extractPricing: {
    source: {
      url: "https://api.qnaigc.com/v1/models",
      type: "api",
      description: "Qiniu AI API — pricing not available via API, omitted",
    },
  ...
  extractLimits: {
    source: {
      url: "https://api.qnaigc.com/v1/models",
      type: "api",
      description: "Qiniu AI API — limits not available via API, omitted",
    },
  ...
  extractModalities: {
    source: {
      url: "https://api.qnaigc.com/v1/models",
      type: "api",
      description: "Qiniu AI API — modalities not available via API, omitted",
    },
  ...
  extractFeatures: {
    source: {
      url: "https://api.qnaigc.com/v1/models",
      type: "api",
      description: "Qiniu AI API — features not available via API, omitted",
    },
  ...
  extractDates: {
    source: {
      url: "https://api.qnaigc.com/v1/models",
      type: "api",
      description: "Qiniu AI API — created timestamp for dates",
    },
    execute: async (models: DiscoveredModel[]): Promise<Map<string, ExtractedDates>> => {
      const datesMap = new Map<string, ExtractedDates>();

      for (const m of models) {
        const raw = m.raw as QiniuModel;
