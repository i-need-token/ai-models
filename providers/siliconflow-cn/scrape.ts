...
// ---------------------------------------------------------------------------
// Raw data types (from SiliconFlow CN API)
// ---------------------------------------------------------------------------

interface SiliconflowCnModel {
  id: string;
  object: string;
  created: number;
  owned_by: string;
...
async function fetchModels(): Promise<SiliconflowCnModel[]> {
  const response = await fetch("https://api.siliconflow.cn/v1/models");
  if (!response.ok) {
    throw new Error(`Failed to fetch SiliconFlow CN models: ${response.status}`);
  }
  const data = (await response.json()) as { data: SiliconflowCnModel[] };
  return data.data;
}
...
const pipeline: ScrapePipeline = {
  discover: {
    source: {
      ...
      type: "api",
      description: "SiliconFlow CN /v1/models API — dynamic model discovery",
    },
  ...
  extractPricing: {
    source: {
      url: "https://api.siliconflow.cn/v1/models",
      type: "api",
      description: "SiliconFlow CN API — pricing not available via API, omitted",
    },
  ...
  extractLimits: {
    source: {
      url: "https://api.siliconflow.cn/v1/models",
      type: "api",
      description: "SiliconFlow CN API — limits not available via API, omitted",
    },
  ...
  extractModalities: {
    source: {
      url: "https://api.siliconflow.cn/v1/models",
      type: "api",
      description: "SiliconFlow CN API — modalities not available via API, omitted",
    },
  ...
  extractFeatures: {
    source: {
      url: "https://api.siliconflow.cn/v1/models",
      type: "api",
      description: "SiliconFlow CN API — features not available via API, omitted",
    },
  ...
  extractDates: {
    source: {
      url: "https://api.siliconflow.cn/v1/models",
      type: "api",
      description: "SiliconFlow CN API — created timestamp for dates",
    },
    execute: async (models: DiscoveredModel[]): Promise<Map<string, ExtractedDates>> => {
      const datesMap = new Map<string, ExtractedDates>();

      for (const m of models) {
        const raw = m.raw as SiliconflowCnModel;
