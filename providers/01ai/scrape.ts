...
const provider = defineProvider({
  ...
  api_docs: "https://docs.lingyiwanwu.com",
  apis: {
...
async function fetchModels(): Promise<YiModel[]> {
  const response = await fetch("https://api.lingyiwanwu.com/v1/models");
  if (!response.ok) {
    throw new Error(`Failed to fetch 01.AI models: ${response.status}`);
  }
  const data = (await response.json()) as { data: YiModel[] };
  return data.data;
}
...
const pipeline: ScrapePipeline = {
  discover: {
    source: {
      ...
      type: "api",
      description: "01.AI /v1/models API — dynamic model discovery",
    },
  ...
  extractPricing: {
    source: {
      url: "https://api.lingyiwanwu.com/v1/models",
      type: "api",
      description: "01.AI API — pricing not available via API, omitted",
    },
  ...
  extractLimits: {
    source: {
      url: "https://api.lingyiwanwu.com/v1/models",
      type: "api",
      description: "01.AI API — limits not available via API, omitted",
    },
  ...
  extractModalities: {
    source: {
      url: "https://api.lingyiwanwu.com/v1/models",
      type: "api",
      description: "01.AI API — modalities not available via API, omitted",
    },
  ...
  extractFeatures: {
    source: {
      url: "https://api.lingyiwanwu.com/v1/models",
      type: "api",
      description: "01.AI API — features not available via API, omitted",
    },
  ...
  extractDates: {
    source: {
      url: "https://api.lingyiwanwu.com/v1/models",
      type: "api",
      description: "01.AI API — created timestamp for dates",
    },
    execute: async (models: DiscoveredModel[]): Promise<Map<string, ExtractedDates>> => {
      const datesMap = new Map<string, ExtractedDates>();

      for (const m of models) {
        const raw = m.raw as YiModel;
