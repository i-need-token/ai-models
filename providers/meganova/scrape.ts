...
// ---------------------------------------------------------------------------
// Raw data types (from MegaNova API)
// ---------------------------------------------------------------------------
...
async function fetchModels(): Promise<MeganovaModel[]> {
  const response = await fetch("https://api.meganova.ai/v1/models");
  if (!response.ok) {
    throw new Error(`Failed to fetch MegaNova models: ${response.status}`);
  }
  const data = (await response.json()) as { data: MeganovaModel[] };
  return data.data;
}
...
const pipeline: ScrapePipeline = {
  discover: {
    source: {
      ...
      type: "api",
      description: "MegaNova /v1/models API — dynamic model discovery",
    },
  ...
  extractPricing: {
    source: {
      url: "https://api.meganova.ai/v1/models",
      type: "api",
      description: "MegaNova API — pricing not available via API, omitted",
    },
  ...
  extractLimits: {
    source: {
      url: "https://api.meganova.ai/v1/models",
      type: "api",
      description: "MegaNova API — limits not available via API, omitted",
    },
  ...
  extractModalities: {
    source: {
      url: "https://api.meganova.ai/v1/models",
      type: "api",
      description: "MegaNova API — modalities not available via API, omitted",
    },
  ...
  extractFeatures: {
    source: {
      url: "https://api.meganova.ai/v1/models",
      type: "api",
      description: "MegaNova API — features not available via API, omitted",
    },
  ...
  extractDates: {
    source: {
      url: "https://api.meganova.ai/v1/models",
      type: "api",
      description: "MegaNova API — created timestamp for dates",
    },
    execute: async (models: DiscoveredModel[]): Promise<Map<string, ExtractedDates>> => {
      const datesMap = new Map<string, ExtractedDates>();

      for (const m of models) {
        const raw = m.raw as MeganovaModel;
