...
// ---------------------------------------------------------------------------
// Raw data types (from Cloudflare Workers AI API)
// ---------------------------------------------------------------------------
...
async function fetchModels(): Promise<CloudflareModel[]> {
  const response = await fetch(
    "https://api.cloudflare.com/client/v4/accounts/{account_id}/ai/v1/models",
  );
  if (!response.ok) {
    throw new Error(`Failed to fetch Cloudflare Workers AI models: ${response.status}`);
  }
  const data = (await response.json()) as { data: CloudflareModel[] };
  return data.data;
}
...
const pipeline: ScrapePipeline = {
  discover: {
    source: {
      ...
      type: "api",
      description: "Cloudflare Workers AI /v1/models API — dynamic model discovery",
    },
  ...
  extractPricing: {
    source: {
      url: "https://api.cloudflare.com/client/v4/accounts/{account_id}/ai/v1/models",
      type: "api",
      description: "Cloudflare Workers AI API — pricing not available via API, omitted",
    },
  ...
  extractLimits: {
    source: {
      url: "https://api.cloudflare.com/client/v4/accounts/{account_id}/ai/v1/models",
      type: "api",
      description: "Cloudflare Workers AI API — limits not available via API, omitted",
    },
  ...
  extractModalities: {
    source: {
      url: "https://api.cloudflare.com/client/v4/accounts/{account_id}/ai/v1/models",
      type: "api",
      description: "Cloudflare Workers AI API — modalities not available via API, omitted",
    },
  ...
  extractFeatures: {
    source: {
      url: "https://api.cloudflare.com/client/v4/accounts/{account_id}/ai/v1/models",
      type: "api",
      description: "Cloudflare Workers AI API — features not available via API, omitted",
    },
  ...
  extractDates: {
    source: {
      url: "https://api.cloudflare.com/client/v4/accounts/{account_id}/ai/v1/models",
      type: "api",
      description: "Cloudflare Workers AI API — created timestamp for dates",
    },
    execute: async (models: DiscoveredModel[]): Promise<Map<string, ExtractedDates>> => {
      const datesMap = new Map<string, ExtractedDates>();

      for (const m of models) {
        const raw = m.raw as CloudflareModel;
...
export async function scrape(): Promise<ScrapeResult> {
  ...
  console.log(`  Cloudflare Workers AI: ${models.length} models`);
